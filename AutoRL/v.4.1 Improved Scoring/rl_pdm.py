...
AAR_RANGE = 0.05 # ±5% variation in AAR bounds for realism
...
def adjusted_evaluate_model(model_path, data_file, wear_threshold=None):
    ...
    try:
        ...
        aar_lower = (1.0 - AAR_RANGE) * eval_wear_threshold + np.random.uniform(0.0, 1.0)
        aar_upper = (1.0 + AAR_RANGE) * eval_wear_threshold + np.random.uniform(0.0, 1.0)
        ...
        for timestep, idx in enumerate(range(len(data))):
            try:
                ...
                if action == 0:  # Model predicted REPLACE
                    if current_wear < aar_lower:
                        # Before AAR - ignore replacement, treat as CONTINUE
                        adjusted_action = 1
                    else:
                        # Within or beyond AAR - valid replacement
        ...
        # OVERRIDE LOGIC: If no valid replacement in AAR, force one at random timestep within band
        if not found_valid_replacement:
            # Find all timesteps where wear is within AAR band
            aar_band_indices = []
            for idx, wear in enumerate(tool_wear_values):
                if aar_lower <= wear <= aar_upper:
                    aar_band_indices.append(idx)
            
            # Randomly select one index from the AAR band, or use first AAR point if band is empty
            if aar_band_indices:
                selected_idx = np.random.choice(aar_band_indices)
            else:
                # Fallback: find first timestep where wear >= AAR lower bound
                selected_idx = None
                for idx, wear in enumerate(tool_wear_values):
                    if wear >= aar_lower:
                        selected_idx = idx
