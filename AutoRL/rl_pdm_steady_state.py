...
def calculate_steady_state_metrics(margins):
    """
    Calculate steady-state metrics for wear margin.
    
    T_ss: Episode number where steady state begins (when margin stabilizes)
    Sigma_ss: Standard deviation of margin in the steady-state region
    
    Steady state is detected when the rolling standard deviation drops below
    a threshold and remains stable.
    
    Returns:
        tuple: (T_ss, Sigma_ss) where both are floats
               Returns (len(margins), 0.0) if no steady state detected
    """
    if not margins or len(margins) < 10:
        # Not enough data
    ...
    # Window size for rolling statistics (20% of episodes, min 10, max 50)
    window_size = max(10, min(50, int(n * 0.2)))
    ...
    # Detect steady state: where rolling std becomes consistently low
    # Threshold: 10% of the overall mean margin or 5.0, whichever is smaller
    mean_margin = np.mean(margins_array)
    std_threshold = min(abs(mean_margin) * 0.1, 5.0)
    
    # Find first point where rolling std drops below threshold and stays below
    t_ss = None
    for i in range(len(rolling_std)):
        if rolling_std[i] < std_threshold:
            # Check if it stays below for next few points (confirmation)
            confirmation_window = min(5, len(rolling_std) - i)
            if all(rolling_std[i+j] < std_threshold * 1.5 for j in range(confirmation_window)):
                t_ss = i + window_size  # Convert back to episode index
                break
    
    # If no steady state detected, use the last 30% of episodes as "steady state"
    if t_ss is None:
        t_ss = int(n * 0.7)
