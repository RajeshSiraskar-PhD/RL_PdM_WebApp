...
def plot_evaluation_results(eval_results, model_name):
    ...
    - Shaded regions: AAR (Allowed Action Region) if present
    """
    fig = make_subplots(
        rows=1, cols=1,
    ...
    # Add AAR shaded region if available
    if 'aar_lower' in eval_results and 'aar_upper' in eval_results:
        aar_lower = eval_results['aar_lower']
        aar_upper = eval_results['aar_upper']
...
st.markdown(' - V.3.1: E.A. = Adjusted eval')
...
with col2:
        ...
        if st.session_state.show_comparison:
            ...
            if st.session_state.get('sensor_fig'):
                with st.expander("📊 Sensor Data Overview", expanded=False):
                    st.plotly_chart(st.session_state.sensor_fig, use_container_width=True)
    ...
    if st.session_state.eval_results is not None:
        ...
        if 'aar_lower' in eval_results and 'aar_upper' in eval_results:
            st.markdown("**Allowed Action Region (AAR):**")
            col_aar1, col_aar2, col_aar3 = st.columns(3)
            with col_aar1:
                st.metric("AAR Lower", f"{eval_results['aar_lower']:.2f}")
            with col_aar2:
                st.metric("Wear Threshold", f"{eval_results['wear_threshold']:.2f}")
            with col_aar3:
                st.metric("AAR Upper", f"{eval_results['aar_upper']:.2f}")
