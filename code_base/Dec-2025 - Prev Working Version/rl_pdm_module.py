...
class Config:
    ...
    EPISODES: int = 10                          # 200 Training episodes
    LEARNING_RATE: float = 0.1                  # 1e-3 Learning rate for optimizers
    GAMMA: float = 0.9                         # 0.99 Discount factor
    ...
    def get_latest_model(cls, agent_type: str, data_file: str = None) -> str:
        ...
        elif agent_type in ['HSD_BASIC', 'HSD BASIC', 'HSD']:
            pattern_files = [f for f in files if '_HSD_Basic_' in f]
        elif agent_type in ['HSD_AM', 'HSD AM']:
            pattern_files = [f for f in files if '_HSD_AM_' in f]
        else:
            pattern_files = []
...
class MT_Env(gym.Env):
    ...
    def __init__(
        ...
        self.df['UL'] = threshold_idx - self.df.index.to_numpy()
...
# --- Train HSD model classical_ml---
def train_HSD_model(data_file: str, save_path: str = None, attention_weights: np.ndarray = None) -> Tuple[Any, Dict]:
    """
    Train a Hidden State Design model on a balanced version of the dataset.
    ...
    model_type_label = "REINFORCE* with Attention" if attention_weights is not None else "REINFORCE*"
    ...
    # Train Classifier
    if attention_weights is not None:
        # Use MLP for Attention-weighted features as it's scale-sensitive
        model = MLPClassifier(hidden_layer_sizes=(64, 32), max_iter=500, random_state=42)
        print(f"  Training MLPClassifier for Attention-weighted features...")
    else:
        # Use Random Forest for Basic version
        model = RandomForestClassifier(n_estimators=100, random_state=42)
        print(f"  Training RandomForestClassifier for Basic features...")
...
def evaluate_all_models(
    ...
    HSD_Basic_model_file: str = None,
    HSD_AM_model_file: str = None
) -> pd.DataFrame:
    """
    ...
    Args:
        ...
        HSD_Basic_model_file: Path to HSD Basic model (optional)
        HSD_AM_model_file: Path to HSD Attention model (optional)
    ...
    if HSD_Basic_model_file is None:
        HSD_Basic_model_file = Config.get_latest_model('HSD_BASIC', test_file)
    if HSD_AM_model_file is None:
        HSD_AM_model_file = Config.get_latest_model('HSD_AM', test_file)
    ...
    models_to_eval = [
        ...
        ('REINFORCE* with Attention', HSD_AM_model_file, 'HSD_AM')
    ]
