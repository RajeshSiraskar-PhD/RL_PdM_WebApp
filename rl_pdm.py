...
class MT_Env(gym.Env):
    ...
    def __init__(self, data_file: str, wear_threshold: float = WEAR_THRESHOLD,
        ...
        self.current_wear = 0.0  # Tool wear counter (independent from time progression)
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        Critical design: current_step ALWAYS increments to ensure episode termination.
        Only the wear counter is reset on replacement, not time progression.
        """
        # Wear increases at each step (simulating tool degradation over time)
        # In real scenario, you'd use the actual wear from the dataset
        self.current_wear = self.original_tool_wear[self.current_step]
        ...
        if action == REPLACE_TOOL:
            ...
            # CRITICAL: Reset wear counter, but DO NOT reset current_step
            # Time marches forward - only the tool state resets
            # This ensures episodes eventually terminate by progressing through data
            self.current_wear = 0.0
        ...
        # This is essential: time progression is independent of replacement decisions
        self.current_step += 1
